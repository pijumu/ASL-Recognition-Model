...
Layer::Layer(const std::string &act_func, int size, Matrix &weights, double *bias_weights): act_func(act_func),
    size(size),
    weights(weights),
...
Layer::Layer(std::string &act_func, int row, int size): act_func(act_func),
                                                        size(size),
                                                        weights(row, size),
...
void Network::back_propagation(double *expected) {
    if (loss_func == CrossEntropy) {
        ...
        for (int i{0}; i < layers[size - 1].size; ++i) {
            layers[size - 1].de_ds[i] = 2 * (layers[size - 1].neurons[i] - expected[i]) * der[i] / static_cast<double>(
                                            size);
        }
    }
